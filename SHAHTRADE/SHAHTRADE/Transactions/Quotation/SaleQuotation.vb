...
Public Class SaleQuotation
    ...
    Sub GETMAX_QUOTE_NO()
        Dim DTTABLE As DataTable = getmax(" isnull(max(QUOTE_no),0) + 1 ", "  SALEQUOTATION ", " AND QUOTE_YEARID = " & YearId)
        If DTTABLE.Rows.Count > 0 Then TXTQUOTENO.Text = DTTABLE.Rows(0).Item(0)
    End Sub
    ...
    Private Sub SaleQuotation_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Sub FILLCMB()
        ...
        fillname(CMBEXTRACHGS, EDIT, " AND (GROUP_SECONDARY = 'INDIRECT EXPENSES' OR GROUP_SECONDARY = 'INDIRECT INCOME' OR GROUP_SECONDARY = 'DIRECT EXPENSES' OR GROUP_SECONDARY = 'DIRECT INCOME')")
    End Sub

    Private Sub SaleQuotation_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If EDIT = True Then
                ...
                Dim OBJPUR As New ClsSALEQUOTATION

                ALPARAVAL.Add(TEMPQUOTENO)
                ...
                If DT.Rows.Count > 0 Then
                    For Each DR As DataRow In DT.Rows
                        ...
                        TXTQUOTENO.Text = TEMPQUOTENO
                        'cmbregister.Text = TEMPREGNAME
                        QUOTEDATE.Text = Format(Convert.ToDateTime(DR("DATE")).Date, "dd/MM/yyyy")
                    ...
                    Dim DTTABLE As DataTable = OBJCMNN.search("ISNULL(QUOTE_UPSRNO, 0) AS SRNO, ISNULL(QUOTE_UPREMARKS, '') AS REMARKS, ISNULL(QUOTE_UPNAME, '') AS NAME, QUOTE_IMGPATH AS IMGPATH", "", "SALEQUOTATION_UPLOAD", "AND SALEQUOTATION_UPLOAD.QUOTE_NO= " & TEMPQUOTENO & "  AND QUOTE_YEARID = " & YearId & " ORDER BY SALEQUOTATION_UPLOAD.QUOTE_UPSRNO")
    ...
    Private Sub cmdok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdok.Click
        ...
        Try
            ...
            ALPARAVAL.Add(Format(Convert.ToDateTime(QUOTEDATE.Text).Date, "MM/dd/yyyy"))
            ...
            If EDIT = False Then
                ...
                TXTQUOTENO.Text = DTTABLE.Rows(0).Item(0)
    ...
    Sub SAVEUPLOAD()
        Try
            Dim OBJORDER As New ClsSALEQUOTATION
    ...
    Sub PRINTREPORT(ByVal QUOTENO As Integer)
        Try
            tempmsg = MsgBox("Wish to Print Sale Quotation?", MsgBoxStyle.YesNo)
            If tempmsg = vbYes Then
                Dim OBJQUOTE As New SaleInvoiceDesign
                OBJQUOTE.MdiParent = MDIMain
                OBJQUOTE.FRMSTRING = "QUOTE"
                OBJQUOTE.WHERECLAUSE = "{SALEQUOTATION.QUOTE_no}=" & Val(QUOTENO) & " and {SALEQUOTATION.QUOTE_yearid}=" & YearId
                OBJQUOTE.Show()
            End If
        Catch ex As Exception
            Throw ex
    ...
    Private Function errorvalid() As Boolean
        ...
        If PBlock.Visible = True Then
            EP.SetError(lbllocked, "Quote is Locked")
            bln = False
        End If
        ...
        If QUOTEDATE.Text = "__/__/____" Then
            EP.SetError(QUOTEDATE, " Please Enter Proper Date")
            bln = False
        Else
            If Not datecheck(QUOTEDATE.Text) Then
                EP.SetError(QUOTEDATE, "Date not in Accounting Year")
                bln = False
            End If
        End If
    ...
    Sub fillgrid()
        ...
        ElseIf GRIDDOUBLECLICK = True Then
            ...
            GRIDQUOTE.Item(GITEMNAME.Index, TEMPROW).Value = CMBITEMNAME.Text.Trim
    ...
    Sub editrow()
        Try
            If GRIDQUOTE.CurrentRow.Index >= 0 And GRIDQUOTE.Item(gsrno.Index, GRIDQUOTE.CurrentRow.Index).Value <> Nothing Then
                ...
                CMBUNIT.Text = GRIDQUOTE.Item(GUNIT.Index, GRIDQUOTE.CurrentRow.Index).Value.ToString
    ...
    Private Sub txtamount_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTAMOUNT.Validating
        If TXTPARTNO.Text.Trim <> "" And CMBITEMNAME.Text.Trim <> "" And TXTMAKE.Text.Trim <> "" And Val(TXTQTY.Text.Trim) > 0 And CMBUNIT.Text.Trim <> "" And Val(TXTRATE.Text.Trim) > 0 And Val(TXTAMOUNT.Text.Trim) > 0 Then
            fillgrid()
            total()
            getsrno(GRIDQUOTE)
        Else
    ...
    Private Sub toolprevious_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles toolprevious.Click
        Try
            GRIDQUOTE.RowCount = 0
Line1:
            TEMPQUOTENO = Val(TXTQUOTENO.Text) - 1
            If TEMPQUOTENO > 0 Then
                EDIT = True
                SaleQuotation_Load(sender, e)
            Else
                CLEAR()
                EDIT = False
            End If
            If GRIDQUOTE.RowCount = 0 And TEMPQUOTENO > 1 Then
                TXTQUOTENO.Text = TEMPQUOTENO
                GoTo Line1
            End If
        Catch ex As Exception
...
LINE1:
            TEMPQUOTENO = Val(TXTQUOTENO.Text) + 1
            GETMAX_QUOTE_NO()
            Dim MAXNO As Integer = TXTQUOTENO.Text.Trim
            CLEAR()
            If Val(TXTQUOTENO.Text) - 1 >= TEMPQUOTENO Then
                EDIT = True
                SaleQuotation_Load(sender, e)
            Else
                CLEAR()
                EDIT = False
            End If
            If GRIDQUOTE.RowCount = 0 And TEMPQUOTENO < MAXNO Then
                TXTQUOTENO.Text = TEMPQUOTENO
                GoTo LINE1
            End If
        Catch ex As Exception
    ...
    Private Sub tstxtbillno_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles tstxtbillno.Validating
        Try
            GRIDQUOTE.RowCount = 0
            TEMPQUOTENO = Val(tstxtbillno.Text)
            If TEMPQUOTENO > 0 Then
                EDIT = True
                SaleQuotation_Load(sender, e)
            Else
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            ...
            If lbllocked.Visible = True Then
                EP.SetError(lbllocked, "Quotation Locked")
                Exit Sub
            End If
    ...
    Private Sub OpenToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OpenToolStripButton.Click
        Try
            Dim objpurinv As New SaleQuotationDetails
    ...
    Private Sub PrintToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        Try
            If EDIT = True Then PRINTREPORT(TEMPQUOTENO)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub

    Private Sub QUOTEDATE_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles QUOTEDATE.GotFocus
        QUOTEDATE.SelectAll()
    End Sub

    Private Sub QUOTEDATE_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles QUOTEDATE.Validating
        Try
            If QUOTEDATE.Text.Trim <> "__/__/____" Then
                'PARSING DATE FORMATS WHETHER THEY ARE PROPER OR NOT
                Dim TEMP As DateTime
                If Not DateTime.TryParse(QUOTEDATE.Text, TEMP) Then
    ...
    Private Sub VALIDTILL_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles VALIDTILL.GotFocus
        VALIDTILL.SelectAll()
    End Sub

    Private Sub VALIDTILL_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles VALIDTILL.Validating
        Try
            If VALIDTILL.Text.Trim <> "__/__/____" Then
                'PARSING DATE FORMATS WHETHER THEY ARE PROPER OR NOT
                Dim TEMP As DateTime
                If Not DateTime.TryParse(VALIDTILL.Text, TEMP) Then
                    MsgBox("Enter Proper Date")
                    e.Cancel = True
                    Exit Sub
                End If
    ...
    Private Sub CMBEXTRACHGS_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBEXTRACHGS.Enter
        Try
            If CMBEXTRACHGS.Text.Trim = "" Then fillname(CMBEXTRACHGS, EDIT, " AND (GROUP_SECONDARY = 'INDIRECT EXPENSES' OR GROUP_SECONDARY = 'INDIRECT INCOME' OR GROUP_SECONDARY = 'DIRECT EXPENSES' OR GROUP_SECONDARY = 'DIRECT INCOME')")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub CMBEXTRACHGS_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBEXTRACHGS.Validating
        Try
            If CMBEXTRACHGS.Text.Trim <> "" Then namevalidate(CMBEXTRACHGS, CMBOTHERCHGSCODE, e, Me, TXTOTHERCHGSADD, " AND (GROUP_SECONDARY = 'INDIRECT EXPENSES' OR GROUP_SECONDARY = 'INDIRECT INCOME' OR GROUP_SECONDARY = 'DIRECT EXPENSES' OR GROUP_SECONDARY = 'DIRECT INCOME')", "INDIRECT EXPENSES")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
