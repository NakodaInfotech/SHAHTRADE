...
Public Class CREDITNOTE
    ...
    Private Sub CMBBILLNO_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBBILLNO.Validating
        Try
            ...
            If CMBBILLNO.Text.Trim <> "" Then
                ...
                Next
                If Not BLN Then
                    MsgBox("Invalid Bill No.", MsgBoxStyle.Critical, "SHAHTRADE")
                    CMBBILLNO.Focus()
                    Exit Sub
                End If
            ...
            If e.RowIndex >= 0 Then
                With GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVCHK").Index)
                    ...
                    Else
                        ...
                        Dim OBJCMN As New ClsCommon
                        Dim DT As DataTable = OBJCMN.search("*", "", " (SELECT ISNULL(INVOICE_PRINTINITIALS,'') AS INVPRINTINITIALS, ISNULL(INVOICE_TOTALQTY,0) AS TOTALPCS, ISNULL(INVOICEMASTER.INVOICE_TOTALTAXABLEAMT,0) AS TAXABLEAMT FROM INVOICEMASTER WHERE INVOICEMASTER.INVOICE_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND INVOICEMASTER.INVOICE_YEARID = " & YearId & " UNION ALL SELECT OPENINGBILL.BILL_INITIALS AS INVPRINTINITIALS, 0 AS TOTALPCS, OPENINGBILL.BILL_AMT AS TAXABLEAMT FROM  OPENINGBILL WHERE OPENINGBILL.BILL_INITIALS = '" & GRIDBILL.Rows(e.RowIndex).Cells(GRIDBILL.Columns("INVBILLINITIALS").Index).Value & "' AND OPENINGBILL.BILL_YEARID = " & YearId & ") AS T", "")
                        If DT.Rows.Count > 0 Then
    ...
    Private Sub CMBNAME_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles CMBNAME.Validated
        Try
            If CMBNAME.Text.Trim <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" ISNULL(STATEMASTER.state_remark, '') AS STATECODE, ISNULL(LEDGERS.ACC_GSTIN,'') AS GSTIN, ISNULL(GROUPMASTER.GROUP_SECONDARY,'') AS SECONDARY ", "", " LEDGERS INNER JOIN GROUPMASTER ON LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN STATEMASTER ON LEDGERS.Acc_stateid = STATEMASTER.state_id ", " and LEDGERS.acc_cmpname = '" & CMBNAME.Text.Trim & "' and (GROUPMASTER.GROUP_SECONDARY = 'SUNDRY DEBTORS' or GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS') and LEDGERS.acc_YEARid = " & YearId)
                If DT.Rows.Count > 0 Then
    ...
    Sub GETHSNCODE()
        Try
            ...
            If CMBSACDESC.Text.Trim <> "" And Convert.ToDateTime(CNDATE.Text).Date >= "01/07/2017" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" TOP 1 ISNULL(HSN_CODE, '') AS HSNCODE, ISNULL(HSN_CGST, 0) AS CGSTPER, ISNULL(HSN_SGST, 0) AS SGSTPER, ISNULL(HSN_IGST, 0) AS IGSTPER", "", " HSNMASTER ", " AND HSNMASTER.HSN_ITEMDESC = '" & CMBSACDESC.Text.Trim & "' AND HSNMASTER.HSN_YEARID='" & YearId & "' ORDER BY HSNMASTER.HSN_ID DESC")
                If DT.Rows.Count > 0 Then
        ...
        Try
            ...
            If TXTADJSRNO.Text.Trim.Length > 0 And Val(TXTADJAMT.Text) > 0 Then
                If CMBPAYTYPE.Text = "Against Bill" And CMBBILLNO.Text.Trim = "" Then
                    MsgBox("Select Bill First", MsgBoxStyle.Critical, "SHAHTRADE")
                    CMBPAYTYPE.Focus()
                    Exit Sub
                End If
                ...
                If CMBBILLNO.Text.Trim <> "" Then
                    For Each ROW As DataGridViewRow In GRIDPAYMENT.Rows
                        If (ROW.Cells(gbillno.Index).Value = CMBBILLNO.Text.Trim And GRIDADJDOUBLECLICK = False) Or (GRIDADJDOUBLECLICK = True And ROW.Cells(gbillno.Index).Value = CMBBILLNO.Text.Trim And ROW.Index <> TEMPADJROW) Then
                            MsgBox("Bill Already present in Grid below", MsgBoxStyle.Critical, "SHAHTRADE")
                            CMBPAYTYPE.Focus()
                            Exit Sub
                        End If
                    Next
