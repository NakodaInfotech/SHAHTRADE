...
Public Class Receipt_Copy
    ...
    '    Private Sub txtamt_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtamt.KeyPress, txttotal.KeyPress
    ...
    '                EP.SetError(ACCDATE, "Enter Proper Date")
    '                BLN = False
    '            ElseIf Not datecheck(ACCDATE.Text) Then
    ...
    '            fillPARTYBANK(CMBPARTYBANK, edit)
    '            ACCDATE.Text = Mydate
    ...
    '                Dim OBJCMN As New ClsCommon
    '                Dim DT As DataTable = OBJCMN.search("ISNULL(PARTYBANKMASTER.PARTYBANK_name, '') AS BANKNAME", "", "   LEDGERS LEFT OUTER JOIN PARTYBANKMASTER ON LEDGERS.Acc_Bankid = PARTYBANKMASTER.PARTYBANK_id AND LEDGERS.Acc_yearid = PARTYBANKMASTER.PARTYBANK_yearid", " and LEDGERS.acc_cmpname = '" & cmbname.Text.Trim & "' and LEDGERS.acc_YEARid = " & YearId)
    '                If DT.Rows.Count > 0 Then
    '                    CMBPARTYBANK.Text = DT.Rows(0).Item("BANKNAME")
    '                End If
    '            End If
    ...
    '                        TXTBILLREMARKS.Text = "Bg. Chqno. " & txtchqno.Text.Trim & " of " & CMBPARTYBANK.Text.Trim & " Against Bill No : " & ROW.Cells(gbillno.Index).Value
    '                    Else
    ...
    '            'DONE BY GULKIT THIS GIVE PROBLEM ON EDIT MODE, IT APPENDS THE CURENT REMARKS EVERY TIME WE EDIT ENTRY
    '            'If edit = False Then
    '            '    txtremarks.Text = TXTBILLREMARKS.Text.Trim & " " & txtremarks.Text.Trim
    '            '    'txtremarks.Text = TXTBILLREMARKS.Text.Trim
    '            'Else
    '            '    txtremarks.Text = txtremarks.Text.Trim
    '            'End If

    '            If edit = False Then txtremarks.Text = TXTBILLREMARKS.Text.Trim & " " & txtremarks.Text.Trim
    ...
    '            gridbill.Columns(i).Width = 80
    '            gridbill.Columns(i).Name = "REGNO"
    '            gridbill.Columns(i).HeaderText = "Reg No"
    '            gridbill.Columns(i).ReadOnly = True
    '            i += 1
    ...
    '                    Dim OBJCMN As New ClsCommon
    '                    Dim DT As DataTable = OBJCMN.search(" T.BALANCE AS BALANCE", "", " (SELECT BILL_INITIALS AS BILLINITIALS, BILL_BALANCE AS BALANCE, BILL_CMPID AS CMPID , BILL_YEARID AS YEARID FROM OPENINGBILL UNION ALL SELECT INVOICE_INITIALS AS BILLINITIALS, INVOICE_BALANCE AS BALANCE, INVOICE_CMPID AS CMPID, INVOICE_YEARID AS YEARID FROM INVOICEMASTER) AS T", " AND T.BILLINITIALS = '" & cmbbillno.Text.Trim & "' AND T.CMPID = " & CmpId & " AND T.YEARID = " & YearId)
    '                    If DT.Rows.Count > 0 Then
    ...
    '                'GETTING AMT OF THE SELECTED BILL
    '                Dim OBJCMN As New ClsCommon
    '                Dim DT As DataTable = OBJCMN.search(" T.BALANCE AS BALAMT", "", " (SELECT BILL_INITIALS AS BILLINITIALS, BILL_BALANCE AS BALANCE, BILL_CMPID AS CMPID , BILL_YEARID AS YEARID FROM OPENINGBILL UNION ALL SELECT INVOICE_INITIALS AS BILLINITIALS, INVOICE_BALANCE AS BALANCE, INVOICE_CMPID AS CMPID , INVOICE_YEARID AS YEARID FROM INVOICEMASTER) AS T", " AND T.BILLINITIALS = '" & cmbbillno.Text.Trim & "' AND T.CMPID = " & CmpId & " AND T.YEARID = " & YearId)
    '                If DT.Rows.Count > 0 Then
    ...
    '                    Dim tempmsg As Integer = MsgBox("Delete Receipt Entry Permanently?", MsgBoxStyle.YesNo, "SHAHTRADE")
    '                    If tempmsg = vbYes Then
    ...
    '                        MsgBox("Chq. No. Already Present with this Bank in Receipt No." & DT.Rows(0).Item(0), MsgBoxStyle.Critical, "SHAHTRADE")
    '                        e.Cancel = True
    '                    End If
    '                End If
    '            End If
