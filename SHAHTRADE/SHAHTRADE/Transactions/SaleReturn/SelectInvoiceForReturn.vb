...
Public Class SelectInvoiceForReturn
    ...
    Sub fillgrid(ByVal WHERE As String)
        Try
            ...
            'Dim DT As DataTable = objclspreq.search("*", "", "(SELECT DISTINCT ISNULL(OPENINGBILL.BILL_NO, 0) AS INVNO, ISNULL(OPENINGBILL.BILL_DATE, GETDATE()) AS INVDATE, 0 AS GDNNO, ISNULL(OPENINGBILL.BILL_DATE, GETDATE()) AS GDNDATE, 0 AS TOTALPCS, 0 AS TOTALMTRS, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(OPENINGBILL.BILL_REGISTERID,0) AS REGID, 'OPENING' AS INVOICETYPE  FROM LEDGERS INNER JOIN OPENINGBILL ON LEDGERS.Acc_id = OPENINGBILL.BILL_LEDGERID WHERE OPENINGBILL.BILL_YEARID = " & YearId & WHERE & " AND OPENINGBILL.BILL_BALANCE > 0 UNION ALL SELECT DISTINCT ISNULL(INVOICEMASTER.INVOICE_NO, 0) AS INVNO, ISNULL(INVOICEMASTER.INVOICE_DATE, GETDATE()) AS INVDATE, ISNULL(INVOICEMASTER.INVOICE_GDNNO, '') AS GDNNO, ISNULL(INVOICEMASTER.INVOICE_GDNDATE, GETDATE()) AS GDNDATE, ISNULL(INVOICEMASTER.INVOICE_TOTALPCS, 0) AS TOTALPCS, ISNULL(INVOICEMASTER.INVOICE_TOTALMTRS, 0) AS TOTALMTRS, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(INVOICE_REGISTERID,0) AS REGID, 'INVOICE' AS INVOICETYPE  FROM LEDGERS INNER JOIN INVOICEMASTER ON LEDGERS.Acc_id = INVOICEMASTER.INVOICE_LEDGERID WHERE INVOICEMASTER.INVOICE_YEARID = " & YearId & WHERE & " AND INVOICEMASTER.INVOICE_BALANCE > 0) AS T", " ORDER BY T.INVNO")
            Dim DT As DataTable = objclspreq.search("*", "", "(SELECT DISTINCT ISNULL(OPENINGBILL.BILL_NO, 0) AS INVNO, ISNULL(OPENINGBILL.BILL_DATE, GETDATE()) AS INVDATE, 0 AS TOTALQTY, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(OPENINGBILL.BILL_REGISTERID,0) AS REGID, 'OPENING' AS INVOICETYPE  FROM LEDGERS INNER JOIN OPENINGBILL ON LEDGERS.Acc_id = OPENINGBILL.BILL_LEDGERID WHERE OPENINGBILL.BILL_YEARID = " & YearId & WHERE & " AND OPENINGBILL.BILL_BALANCE > 0 UNION ALL SELECT DISTINCT ISNULL(INVOICEMASTER.INVOICE_NO, 0) AS INVNO, ISNULL(INVOICEMASTER.INVOICE_DATE, GETDATE()) AS INVDATE, ISNULL(INVOICEMASTER.INVOICE_TOTALQTY, 0) AS TOTALQTY, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(INVOICE_REGISTERID,0) AS REGID, 'INVOICE' AS INVOICETYPE  FROM LEDGERS INNER JOIN INVOICEMASTER ON LEDGERS.Acc_id = INVOICEMASTER.INVOICE_LEDGERID  WHERE INVOICEMASTER.INVOICE_YEARID = " & YearId & WHERE & " AND INVOICEMASTER.INVOICE_BALANCE > 0) AS T", " ORDER BY T.INVNO")
            ...
            If DT.Rows.Count > 0 Then
                ...
                GRIDRET.Columns("TOTALQTY").Width = 100 'TOTAL QTY
                GRIDRET.Columns("TOTALQTY").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
                GRIDRET.Columns("TOTALQTY").DefaultCellStyle.Format = "N2"
                'I = I + 1
                ...
                'I = I + 1
                GRIDRET.Columns("REGID").Visible = False 'REGID
                ' I = I + 1
                GRIDRET.Columns("INVOICETYPE").Visible = False 'INVTYPE
                'I = I + 1
        ...
        Try
            ...
            For Each ROW As DataGridViewRow In GRIDRET.Rows
                If ROW.Cells(0).Value = True Then
                    ...
                    DT.Rows.Add(ROW.Cells("INVNO").Value, ROW.Cells("REGID").Value, ROW.Cells("INVOICETYPE").Value)
